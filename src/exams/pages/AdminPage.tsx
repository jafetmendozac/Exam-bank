...
export default function AdminPage() {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const approved = allExams.filter((e) => e.status === "approved");
      const rejected = allExams.filter((e) => e.status === "rejected");
      const totalDownloads = allExams.reduce((sum, e) => sum + (e.downloads || 0), 0);
      ...
      // Top cursos
      const courseCounts = allExams.reduce((acc, exam) => {
      ...
      // Top profesores
      const teacherCounts = allExams.reduce((acc, exam) => {
